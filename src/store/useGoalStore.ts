    ...
    persist(
      (set, get) => ({
        ...
        handleStepSave: (updatedStep: Step) => {
          ...
          if (editingStep) {
            newSteps = steps.map((s) =>
              s.id === editingStep.id ? updatedStep : s
            );
          } else {
        ...
        initializeSteps: (steps) =>
          set({ steps, currentStepIndex: 0, stepsData: {} }),
        moveToNextStep: () =>
          set((state) => {
            const maxStep =
              state.onCompleteAction?.type === "summary"
                ? state.steps.length
        ...
        setIsProcessingLLM: (isProcessing) =>
          set({ isProcessingLLM: isProcessing }),
